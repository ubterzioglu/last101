...
import { getPublishedNewsArticles } from '@/lib/public-news';
...
export default async function HaberlerPage() {
  const articles = await getPublishedNewsArticles(48);
          ...
          {articles.length === 0 ? (
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.03] px-6 py-14 text-center">
              ...
              <p className="mt-4 text-sm leading-7 text-white/68">
                `/admin/home/news` üzerinden bir haberi `Yayında` durumunda oluşturduğunuzda burada görünür.
              </p>
            </div>
          ) : (
