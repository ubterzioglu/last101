...
export default function TavlaClient() {
  ...
  const loadParticipants = useCallback(async () => {
    try {
      ...
      const publicResult = await client.from('tavla_participants_public').select('*').order('created_at', { ascending: false });
      if ((publicResult.error as { code?: string } | null)?.code === 'PGRST205') {
        const mainResult = await client.from('tavla_participants').select('*').order('created_at', { ascending: false });
      ...
      const bracketPublic = await client.from('tavla_bracket_public').select('*').order('slot_index', { ascending: true });
      if ((bracketPublic.error as { code?: string } | null)?.code === 'PGRST205') {
        const bracketMain = await client.from('tavla_bracket').select('*').order('slot_index', { ascending: true });
  ...
  useEffect(() => {
    ...
    getDevUserClient().then(client => {
      ...
      supabase = client;
      client
        .channel('tavla_changes')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'tavla_participants' }, () => { loadParticipants(); })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'tavla_bracket' }, () => { loadParticipants(); })
        .subscribe();
    }).catch(() => {/* ignore */});
    ...
    return () => {
      cancelled = true;
      if (supabase) {
        supabase.channel('tavla_changes').unsubscribe().catch(() => {/* ignore */});
      }
    };
  }, [loadParticipants]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const response = await fetch('/api/tavla-register', {
        method: 'POST',
      ...
      if (!response.ok) {
        if (response.status === 409) { showStatus('Bu isimle zaten kayıtlı!', 'error'); return; }
        if (response.status === 429) { showStatus('Çok hızlı deneme yaptınız. Lütfen biraz bekleyin.', 'error'); return; }
        throw new Error(payload?.error || 'Kayıt hatası');
      }
      showStatus(`Tebrikler ${name}! Turnuvaya başarıyla kayıt oldunuz.`, 'success');
      setFullName('');
      setWhatsapp('');
      loadParticipants();
    } catch {
  ...
  const css = `
    ...
    .tavla-wrap { max-width: 1200px; margin: 0 auto; padding: 0 4px 32px; display: flex; flex-direction: column; gap: 24px; }
  ...
  return (
    <>
      <style>{css}</style>
      <div className="tavla-wrap">
        <div className="card hero-card">
          <div className="hero-domain">almanya101.de</div>
          <div className="trophy-icon">🏆</div>
          <h2>1. Tavla Turnuvası</h2>
          <p>Şampiyon olmak için hazır mısın? Kaydol ve rakiplerini yen!</p>
        </div>

        <div className="card rules-card">
          <h3 className="section-title">📋 Turnuva Kuralları</h3>
          <div className="rules-grid">
            <p><strong style={{ color: 'var(--text-primary)' }}>🎮 Platform:</strong> Oyunlar iki kişinin anlaştığı herhangi bir platformda oynanabilir.</p>
            <p><strong style={{ color: 'var(--text-primary)' }}>📸 Doğrulama:</strong> Kazanan kişi ekran görüntüsü alarak oyun durumunu belirtmelidir.</p>
            <p><strong style={{ color: 'var(--text-primary)' }}>🏆 Kazanma:</strong> 5 sayıya ulaşan kazanır.</p>
            <p><strong style={{ color: 'var(--text-primary)' }}>⏰ Süre:</strong> 1 hafta içinde tamamlanmayan maçlarda iki kişi de turnuvadan elenir.</p>
            <p><strong style={{ color: 'var(--google-yellow)' }}>🎁 Ödül:</strong> Kazananın ödülü <strong>1 aylık Blackbox.ai üyeliğidir!</strong></p>
          </div>
        </div>

        <div className="card form-card">
          <h3 className="section-title">🎮 Turnuvaya Kayıt Ol</h3>
          {statusMsg && <div className={`status-message ${statusMsg.type}`}>{statusMsg.text}</div>}
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              <label htmlFor="tavla-fullName">Ad Soyad *</label>
              <input type="text" id="tavla-fullName" placeholder="Örn: Ahmet Yılmaz" required value={fullName} onChange={e => setFullName(e.target.value)} />
            </div>
            <div className="form-group">
              <label htmlFor="tavla-whatsapp">WhatsApp Numarası *</label>
              <input type="tel" id="tavla-whatsapp" placeholder="Örn: +49 171 1234567" required value={whatsapp} onChange={e => setWhatsapp(e.target.value)} />
            </div>
        ...
        <div className="card bracket-card">
          <h3 className="section-title">🎯 Playoff Eşleşmeleri</h3>
