...
const VOTE_SUBMIT_API_URL = '/api/event-e1-submit';
const VOTE_RESULTS_API_URL = '/api/event-e1-results';
const QUESTION_SUBMIT_API_URL = '/api/event-e1-question-submit';
const QUESTION_LIST_API_URL = '/api/event-e1-question-list';

const CHOICE_KEYS = ['tarih1', 'tarih2', 'tarih3'] as const;
const CHOICE_LABELS: Record<string, string> = {
  tarih1: 'Tarih1 : 26 Şubat Perşembe\n21:00 TR - 19:00 DE',
  tarih2: 'Tarih2 : 27 Şubat Cuma\n21:00 TR - 19:00 DE',
  tarih3: 'Tarih3 : 2 Mart Pazartesi\n21:00 TR - 19:00 DE',
};
...
export function E1Client() {
  ...
  const loadVoteResults = useCallback(async () => {
    ...
    try {
      ...
      const max = Math.max(...CHOICE_KEYS.map((k) => Number(newCounts[k] || 0)), 1);
      setCounts(newCounts);
      setMaxCount(max);
      setResultsMeta(
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      const error = err as Error;
      setQuestionsError(error.message || 'Sorular yüklenemedi.');
    } finally {
  ...
  const handleVoteSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedDates.length) {
      setVoteStatusMsg('En az bir tarih seçmelisiniz.');
      setVoteStatusType('error');
      return;
    }
    ...
    try {
      ...
      setVoteName('');
      setSelectedDates([]);
      setVoteStatusMsg('Tarih seçimin alındı.');
  ...
  const handleQuestionSubmit = async (e: React.FormEvent) => {
    ...
    if (!question) {
      setQuestionStatusMsg('Soru zorunlu.');
      setQuestionStatusType('error');
      return;
    }
    if (question.length > 500) {
      setQuestionStatusMsg('Soru en fazla 500 karakter olabilir.');
      setQuestionStatusType('error');
      return;
    }

    setQuestionSubmitting(true);
    setQuestionStatusMsg('Soru kaydediliyor...');
    ...
    try {
      ...
      setQuestionText('');
      setQuestionStatusMsg('Soru gönderildi.');
      setQuestionStatusType('success');
      await loadQuestions();
    } catch (err) {
      const error = err as Error;
      setQuestionStatusMsg(error.message || 'Soru gönderilirken hata oluştu.');
      setQuestionStatusType('error');
    } finally {
      ...
      <DevUserShell backHref="/devuser/dev" backLabel="← dashboard'a dön" frameVariant="compact">
        <div className="container1">
          <div className="card hero-card1">
            ...
            <h3 style={{ marginBottom: 8 }}>almanya101.de ilk etkinliğini gurula sunar!</h3>
            <p style={{ color: 'rgba(255,255,255,.95)' }}>
              Almanya ile ilgili sorularınızı cevaplamaya çalışıyoruz!
            </p>
            <p style={{ color: 'rgba(255,255,255,.95)', marginTop: 6 }}>
              Tarih seçimini ve sorunu ayrı ayrı gönderebilirsin.
            </p>
          </div>

          <div className="card">
            <div className="head-row">
              <h4>Tarih Seçim Sonuçları</h4>
                ...
                return (
                  <article className="result-card" key={key}>
                    <div className="result-row">
                      <span className="date-label">{CHOICE_LABELS[key]}</span>
          ...
          <div className="card">
            <h4 style={{ marginTop: 0, marginBottom: 10, color: '#fff' }}>Tarih Seçimi Gönder</h4>
            <form onSubmit={handleVoteSubmit} className="stack-form" noValidate>
              ...
              <div>
                <label className="field-label">Tarih Seç (birden fazla seçilebilir)</label>
              ...
              />
              <button id="voteSubmitBtn" className="btn btn-primary" type="submit" disabled={voteSubmitting}>
                {voteSubmitting ? 'Gönderiliyor...' : 'Tarih Seçimini Gönder'}
              </button>
              {voteStatusMsg && (
          ...
          <div className="card">
            <h4 style={{ marginTop: 0, marginBottom: 10, color: '#fff' }}>Soru Gönder</h4>
            <form onSubmit={handleQuestionSubmit} className="stack-form" noValidate>
              <div>
                <label className="field-label" htmlFor="questionInput">Sorunuz</label>
                <textarea
                  id="questionInput"
              ...
              <button className="btn btn-primary" type="submit" disabled={questionSubmitting}>
                {questionSubmitting ? 'Gönderiliyor...' : 'Soruyu Gönder'}
              </button>
              {questionStatusMsg && (
          ...
          <div className="card">
            ...
            {questionsLoading ? (
              ...
              <div className="empty-state">Henüz soru yok.</div>
            ) : (
