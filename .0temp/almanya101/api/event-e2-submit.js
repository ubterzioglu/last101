...
function normalizeTopicChoices(value) {
  ...
  for (const item of source) {
    ...
    for (const part of parts) {
      if (ALLOWED_TOPIC_KEYS.has(part)) unique.add(part);
    }
  }
...
export default async function handler(req, res) {
  ...
  const selectedTopics = normalizeTopicChoices(body.selected_topics);

  if (!anonymous && !fullName) {
    return res.status(400).json({ error: 'Ad soyad zorunlu (anonim degilse).' });
  }
  if (!selectedTopics.length) {
    return res.status(400).json({ error: 'En az bir konu secimi zorunlu.' });
  }
  ...
  try {
    ...
    if (error) {
      console.error('event-e2-submit insert failed:', error);
      return res.status(500).json({ error: mapInsertError(error) });
    }

    return res.status(200).json({ ok: true, id: data?.id || null });
  } catch (error) {
    console.error('event-e2-submit failed:', error);
