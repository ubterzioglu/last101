...
function checkRateLimit(req) {
  const now = Date.now();
  const key = `meeting-attendance:${getClientIp(req)}`;
...
function mapInsertError(error) {
  ...
  if (
    safeMessage.includes('meeting_attendance') &&
    (safeMessage.includes('schema cache') || safeMessage.includes('does not exist') || safeMessage.includes('could not find'))
  ) {
    return {
      status: 503,
      error: 'Veritabani hazir degil. Supabase migration calistirin ve tekrar deneyin.',
    };
  }
...
export default async function handler(req, res) {
  ...
  if (availableDates.length === 0) {
    return res.status(400).json({ error: 'En az bir tarih secilmeli' });
  }
  ...
  try {
    ...
    console.error('meeting-attendance-submit failed:', error);
